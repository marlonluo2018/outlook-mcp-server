    ...
    with OutlookSessionManager() as session:
        while retry_count < 3:
            try:
                ...
                logger.info(
                    f"Processing up to {min(total_items, MAX_EMAILS)} emails from {total_items} total"
                )
                ...
                for i in range(1, total_items + 1):
                    ...
                    try:
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Failed to parse date for email at index {i}: {date_error}"
                            )
                            continue
                        ...
                        raw_sender_name = safe_encode_text(
                            getattr(item, "SenderName", "Unknown Sender"), "sender_name"
                        )
                        # Clean sender name
                        ...
                        failed_count += 1
                        logger.warning(
                            f"Failed to process email at index {i}: {type(e).__name__}: {str(e)}"
                        )
                        continue
