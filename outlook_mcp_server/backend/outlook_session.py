...
class OutlookSessionManager:
    ...
    def create_folder(self, folder_name: str, parent_folder_name: Optional[str] = None) -> str:
        ...
        try:
            ...
            if "/" in folder_name:
                ...
                if len(path_parts) > 3:
                    raise ValueError(
                        "Maximum 3 folder levels supported (e.g., 'Inbox/SubFolder1/SubFolder2')"
                    )
                ...
                for folder in parent_folder.Folders:
                    if folder.Name == folder_name:
                        raise ValueError(
                            f"Folder '{folder_name}' already exists in '{parent_folder.Name}'"
                        )

                # Create the folder
                new_folder = parent_folder.Folders.Add(folder_name)
                folder_path = (
                    f"{parent_folder.Name}/{folder_name}"
                    if parent_folder.Name != "Inbox"
                    else folder_name
                )
            ...
            try:
                ...
                if not self._validate_email_item(email_item):
                    if attempt < max_attempts - 1:
                        logger.warning(
                            f"Email item validation failed on attempt {attempt + 1}, retrying..."
                        )
                ...
                logger.info(
                    f"Successfully moved email '{email_subject}' to folder '{target_folder_name}'"
                )
                ...
                if error_code == -2147352567:
                    if attempt < max_attempts - 1:
                        ...
                        logger.error(
                            f"Failed to move email after {max_attempts} attempts due to COM access error"
                        )
                        raise RuntimeError(
    ...
    def move_folder(self, source_folder_path: str, target_parent_path: str) -> str:
        ...
        try:
            ...
            if (
                len(source_parts) > 4
            ):  # Allow up to 4 levels for mailbox paths like "mailbox@domain.com/Inbox/Folder/SubFolder"
            ...
            for folder in target_parent.Folders:
                if folder.Name == source_folder.Name:
                    raise ValueError(
                        f"Folder '{source_folder.Name}' already exists in '{target_parent_path}'"
                    )
            ...
            logger.info(
                f"Successfully moved folder '{source_folder_path}' to '{new_path}' with {email_count} emails"
            )
            ...
            try:
                ...
                if error_code == -2147352567:
                    if attempt < max_attempts - 1:
                        ...
                        logger.error(
                            f"Failed to move email to Deleted Items after {max_attempts} attempts due to COM access error"
                        )
                        raise RuntimeError(
                ...
                try:
                    ...
                    for category in category_list:
                        for available_policy in available_policies:
                            if (
                                category.lower() == available_policy.lower()
                                or category in available_policy
                                or available_policy in category
                            ):
                                manually_assigned_policies.append(available_policy)
                                print(
                                    f"DEBUG: Found manually assigned policy in Categories: '{available_policy}'"
                                )
                                break
                except Exception as e:
    ...
    def get_available_policies(self) -> list:
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Checking up to {min(items.Count, max_sample)} emails for retention policies..."
                )
                ...
                try:
                    ...
                    for idx, item in enumerate(item_list):
                        try:
                                ...
                                try:
                                    if hasattr(item, prop):
                                        ...
                                        if retention_policy and retention_policy != "Unknown":
                                            logger.debug(
                                                f"Found retention policy '{retention_policy}' using property '{prop}' on item {idx}"
                                            )
                                            break
                                except:
                                    continue
                            ...
                            if (
                                retention_policy
                                and retention_policy != "Unknown"
                                and retention_policy not in found_retention_policies
                            ):
                ...
                logger.info(
                    f"Found {len(found_retention_policies)} retention policies from email sampling"
                )
            ...
            try:
                ...
                for i in range(1, stores.Count + 1):
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Could not access policy templates from store {store.DisplayName}: {str(e)}"
                        )
    ...
    def assign_policy(self, email_id: str, policy_name: str) -> str:
        ...
        try:
            ...
            for p in available_policies:
                if (
                    policy_name.lower() == p.lower()
                    or policy_name.lower() in p.lower()
                    or p.lower() in policy_name.lower()
                ):
                    matching_policy = p
                    break
            ...
            if matching_policy:
                ...
                try:
                    ...
                    email_item.Save()
                    logger.info(
                        f"Successfully assigned Exchange retention policy '{matching_policy}' via direct property assignment"
                    )
                ...
                try:
                    ...
                    email_item.Save()
                    logger.info(
                        f"Successfully assigned Exchange retention policy '{matching_policy}' via Categories"
                    )
                ...
                try:
                    ...
                    for prop_name in alternative_properties:
                        try:
                            ...
                            email_item.Save()
                            logger.info(
                                f"Successfully assigned retention policy '{matching_policy}' via {prop_name}"
                            )
                ...
                logger.error(
                    f"All retention policy assignment methods failed. Tried: {available_methods}"
                )
            ...
            # Check if this looks like an enterprise policy request (no hardcoded policy names)
                ...
                policy_name.lower().endswith(" (enterprise)") or "enterprise" in policy_name.lower()
            ):
                ...
                for p in available_policies:
                    if (
                        base_policy_name.lower() == p.lower()
                        or base_policy_name.lower() in p.lower()
                        or p.lower() in base_policy_name.lower()
                    ):
                        matching_policy = p
                        break
                ...
                try:
                    ...
                    if policy_prop:
                        ...
                        try:
                            policy_prop = email_item.ItemProperties.Add(
                                "enterprise_policy", 1
                            )  # olText
                            policy_prop.Value = base_policy_name
                        except:
                            ...
                            if not policy_prop:
                                ...
                                try:
                                    policy_prop = email_item.UserProperties.Add(
                                        "enterprise_policy", 1, False, 0
                                    )
                                    policy_prop.Value = base_policy_name
                                except:
                    ...
                    try:
                                    ...
                                    for prop in item.UserProperties:
                                        if prop.Name.lower() == "enterprise_policy" and prop.Value:
                                            if hasattr(item, "ExpiryTime") and item.ExpiryTime:
                                                ...
                                                logger.info(
                                                    f"Found existing enterprise policy expiry pattern: {expiry_pattern}"
                                                )
                                                break
                                sample_count += 1
                        except Exception as e:
                            logger.debug(
                                f"Could not sample existing emails for expiry pattern: {str(e)}"
                            )
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Could not set ExpiryTime (method 1: {str(expiry_error)}, method 2: {str(expiry_error2)})"
                            )
                            # Continue anyway - Categories and UserProperty should be sufficient

                    email_item.Save()
                    logger.info(
                        f"Assigned enterprise policy '{base_policy_name}' to email: '{email_subject}'"
                    )
            ...
            if sensitivity_value is None:
                raise ValueError(
                    f"Policy '{policy_name}' not found. Available policies: {available_policies}"
                )
